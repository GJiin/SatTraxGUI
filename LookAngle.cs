...
namespace SatTraxGUI
{
    public class PassData
    {
        ...
        /// a single pass. Given a set of lookAngle Az/EL Events, this method will
        /// calculate a set of AZ/EL/CL look angle events.
        public const string Version = "0.1";

        /// <summary>
        ///     1 - Calls LoadPassData
        ...
        {
            var pass = LoadPassData(listScriptAzEl.ToArray());
            ...
            return LookAngle.ProcessPass(pass);
        }

        /// <summary>
        ///     Load data and precompute SIN/COS for each track Az/EL as wel as
        ///     the overall pass data - MaxEl, Az at Max El, Motion/Direction,
        ///     number of events
        ...
        /// <returns></returns>
        public static PassData LoadPassData(string[] lines)
        {
            var pass = new PassData(new List < LookAngle >(lines.Length));
            ...
            foreach ( var line in lines ) //Parallel.ForEach(lines, line =>
            {
                ...
                var azelcl = new LookAngle(name, date, az, el, cl);
            ...
            var motion = ( pass.rad_AzAtMaxEl - pass.listAzelcl[0].trackAz ).CheckDiff();
        ...
        public List < LookAngle > listAzelcl;
        public double MaxEl;
        ...
        public PassData()
        {
            listAzelcl = new List < LookAngle >();
        }

        public PassData(int capacity)
        {
            listAzelcl = new List < LookAngle >(capacity);
        }

        public PassData(IEnumerable < LookAngle > listAzElCls)
        {
            listAzelcl = new List < LookAngle >(listAzElCls);
        }
        ...
        public static void Serialize(List < LookAngle > aec, string path)
        {
            var writer = new XmlSerializer(typeof( List < LookAngle > ));
            var file = File.Create(path);
            //            writer.Serialize(file, aec);
            file.Close();
        }

        public static List < LookAngle > Deserialize(string path)
        {
            var reader = new XmlSerializer(typeof( LookAngle[] ));
            var file = new StreamReader(path);
            //   var aec = (List<LookAngle>)reader.Deserialize(file);
            file.Close();
            return null; // aec;
        }
    ...
    public class LookAngle
    {
        ...
        public LookAngle(string name, DateTime eciDateTime, double az, double el, double cl)
        {
            this.name = name;
            eciDate = eciDateTime;
        ...
        public static List < string > ProcessPass(PassData pass)
        {
            ////////////////////////////////////////////////
            ...
            switch ( Form1.TrackType )
            {
                case Form1.TrackTYPE.Standard:
                    ...
                    WriteAzElCl(pass.listAzelcl, Form1.DATA_DIRECTORY + std_path);
                ...
                case Form1.TrackTYPE.Overhead:
                    ...
                    pass.listAzelcl[0].pedCl = ( Math.PI / 2 - pass.MaxEl ) * PassData.Direction;
                ...
                case Form1.TrackTYPE.Tilt:
                    ...
                    // Compute parameters based on event period
                    //
                    paramAzVelLimit = ( pass.MaxEl * 3.829 - 3.79115 ).DegreeToRadian() * PassData.sec_EventPeriod;
                    var xparamAzVelLimit = 2.01.DegreeToRadian() * PassData.sec_EventPeriod;

                    paramAzAcc = ( pass.MaxEl * .0957 - 0.0995 ).DegreeToRadian() * Square(PassData.sec_EventPeriod);
                    var xparamAzAcc = .04.DegreeToRadian() * Square(PassData.sec_EventPeriod);

                    paramElStart = ( pass.MaxEl * -14.351 + 77.46681 ).DegreeToRadian();
                    var xparamElStart = 55.0.DegreeToRadian();
            ...
            WriteAzElCl(pass.listAzelcl, Form1.DATA_DIRECTORY + path);
        ...
        public void ProcessRow(LookAngle prev)
        {
        ...
        private void PredictAzVelocity(LookAngle prev)
        {
            trackAzVel = ( trackAz - prev.trackAz ).CheckDiff();
            azErr = ( prev.pedAz - prev.trackAz ).CheckDiff();
            ...
            switch ( State )
            {
                ...
                case 3:
                    ...
                    if ( Math.Abs(azErr) < Math.Abs(prev.azErr) )
                    {
                        azAcc = Square(prev.azVel - trackAzVel) / azErr;
        ...
        private static int State_DEBUG;
        ...
        private static double paramAzVelLimit;
        private static double paramAzAcc;
        private static double paramElStart;

        #endregion Fields

        #region Helpers

        private void PredictOverheadMotion(LookAngle prev)
        {
            deltaUD1 = -prev.pedSinEl * ( prev.pedCosAz * deltaX + prev.pedSinAz * deltaY ) - prev.pedCosEl * deltaZ;
            pedEl = prev.pedEl + deltaUD1 * 0.99999;
            pedAz = prev.pedAz;
            pedCl = prev.pedCl;
        }

        public void DeltaXYZ(LookAngle prev)
        {
        ...
        public static void WriteAzElCl(List<LookAngle> listAzelcl, string path)
        {
